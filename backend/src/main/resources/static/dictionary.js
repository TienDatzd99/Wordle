// English 5-letter words dictionary
const ENGLISH_WORDS = [
    ...
    "CRIME", "CROSS", "CROWD", "CROWN", "CRUDE", "CURVE", "CYCLE", "DAILY", "DANCE", "DATUM",
    ...
    "TAKEN", "TASTE", "TAXES", "TEACH", "TEAMS", "TERMS", "TEXAS", "THANK", "THEFT", "THEIR",
    "THEME", "THERE", "THESE", "THICK", "THING", "THINK", "THIRD", "THOSE", "THREE", "THREW",
    "THROW", "THUMB", "TIGER", "TIGHT", "TIRED", "TITLE", "TODAY", "TOPIC", "TOTAL", "TOUCH",
...
// Dictionary class for word validation
class WordDictionary {
    constructor() {
        this.words = new Set(ENGLISH_WORDS);
    }
    ...
    // Load from external API if needed
    async loadFromAPI() {
        try {
            const response = await fetch('/api/dictionary/words');
            if (response.ok) {
                const apiWords = await response.json();
                apiWords.forEach(word => this.words.add(word.toUpperCase()));
            }
        } catch (error) {
